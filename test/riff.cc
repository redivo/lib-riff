...
TEST(Subchunk, SetPayloadOverrideOK)
{
    riffSubChunk_t subchunk;
    int payload1[3];
    int payload2[5];
...
TEST(Chunk, AddTwoSubchunkOK)
{
    riffSubChunk_t subchunk1, subchunk2;
    riffChunk_t chunk;
    ASSERT_EQ(riffChunkCreateWithType(&chunk, RIFF_FMT_TYPE_WAVE), RIFF_ERR_NONE);

    /* Create the subchunks */
    subchunk1 = riffSubChunkCreate("SC 1");
    int payload[3];
    payload[0] = 5;
    payload[1] = 6;
    payload[2] = 7;
    ASSERT_EQ(riffSubchunkSetPayload(&subchunk1, &payload, sizeof(payload)), RIFF_ERR_NONE);

    subchunk2 = riffSubChunkCreate("SC 2");
    payload[0] = 8;
    payload[1] = 9;
    payload[2] = 10;
    ASSERT_EQ(riffSubchunkSetPayload(&subchunk2, &payload, sizeof(payload)), RIFF_ERR_NONE);

    /* Add the subchunk into the chunk */
    ASSERT_EQ(riffAddSubchunk(&chunk, subchunk1), RIFF_ERR_NONE);
    ASSERT_EQ(riffAddSubchunk(&chunk, subchunk2), RIFF_ERR_NONE);
}
